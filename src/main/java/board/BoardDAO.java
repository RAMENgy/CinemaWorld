...
public class BoardDAO {
	...
	public void insertNotice(BoardDTO bDTO) {
		try {
			...
			con=getConnection();
			//3 sql
			String sql="insert into board(num, name, pass, subject, notice, readcount, date, file) values(?,?,?,?,?,?,?,?)";
			pstmt=con.prepareStatement(sql);
			pstmt.setInt(1, bDTO.getNum());
			pstmt.setString(2, bDTO.getName());
			pstmt.setString(3, bDTO.getPass());
			pstmt.setString(4, bDTO.getSubject());
			pstmt.setString(5, bDTO.getNotice());
			pstmt.setInt(6, bDTO.getReadcount());
			pstmt.setTimestamp(7, bDTO.getDate());
			//file 
			pstmt.setString(8, bDTO.getFile());
	...
	// 추천 영화 추가
	public void insertMovie(BoardDTO bDTO) {
		try {
			//1,2 디비연결
			con=getConnection();
			//3 sql
			String sql="insert into board(num, name, pass, subject, movie, readcount, date, file) values(?,?,?,?,?,?,?,?)";
			pstmt=con.prepareStatement(sql);
			pstmt.setInt(1, bDTO.getNum());
			pstmt.setString(2, bDTO.getName());
			pstmt.setString(3, bDTO.getPass());
			pstmt.setString(4, bDTO.getSubject());
			pstmt.setString(5, bDTO.getMovie());
			pstmt.setInt(6, bDTO.getReadcount());
			pstmt.setTimestamp(7, bDTO.getDate());
			//file 
			pstmt.setString(8, bDTO.getFile());
...
//			String sql="select * from board order by num desc";
			...
			String sql="select * from board where content is not null order by num desc limit ?,?";
			pstmt=con.prepareStatement(sql);
			...
			while(rs.next()) {
				...
				bDTO.setMovie(rs.getString("movie"));
				bDTO.setNotice(rs.getString("notice"));
...
//			String sql="select * from board order by num desc";
			// String sql="select * from board order by num desc limit 시작하는행번호-1,몇개";
			// String sql="select * from board order by num desc limit startRow-1,pageSize";
			String sql="select * from board where notice is not null order by num desc limit ?,?";
			pstmt=con.prepareStatement(sql);
			pstmt.setInt(1, startRow-1);
			pstmt.setInt(2, pageSize);
			// 4 실행 => 결과 저장
...
//					String sql="select * from board where subject like '%검색어%'  order by num desc limit ?,?";
	...
	public List getMovieList(int startRow,int pageSize) {
		// import java.util.List; 자바API 배열
		// import java.util.ArrayList; 자바API 배열
		// 인터페이스 List 상속받은 클래스 ArrayList
		List<BoardDTO> boardList=new ArrayList<BoardDTO>();
...
//			String sql="select * from board order by num desc";
			// String sql="select * from board order by num desc limit 시작하는행번호-1,몇개";
			// String sql="select * from board order by num desc limit startRow-1,pageSize";
			String sql="select * from board where movie is not null order by num desc limit ?,?";
			pstmt=con.prepareStatement(sql);
			pstmt.setInt(1, startRow-1);
			pstmt.setInt(2, pageSize);
			// 4 실행 => 결과 저장
